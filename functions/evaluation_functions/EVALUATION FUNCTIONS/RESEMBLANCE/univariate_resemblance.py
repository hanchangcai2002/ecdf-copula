...
def basic_stats(data) :
    ...
    """Compute basic descriptive statistics (mean and std) for each numerical attribute.

    Parameters
    ----------
    data : pandas.core.frame.DataFrame
        The dataframe to compute statistics on

    Returns
    -------
    pandas.core.frame.DataFrame
        a single-column dataframe indexed by numerical column names, with values formatted as 'mean (std)'
    """

    #get list of numerical column names
    num_cols = data.select_dtypes(include=['int64', 'float64']).columns

    #compute mean and std per column, format as 'mean (std)'
    summary = data[num_cols].agg(['mean', 'std']).round(4)
    result = summary.loc['mean'].map(str) + ' (' + summary.loc['std'].map(str) + ')'
...
def mahalanobis_distances(real, synthetic) :

    """Compute the Mahalanobis distances between real data attributes and synthetic data attributes independently.
    For each numerical column, the univariate Mahalanobis distance is computed as the absolute difference
    between means divided by the standard deviation of the real data.
    ...
    #loop to perform the distances for each attribute
    for c in num_cols :
        std_real = np.std(real[c].values)
        if std_real == 0 :
            dists.append(np.nan)
        else :
            dists.append(np.abs(np.mean(real[c].values) - np.mean(synthetic[c].values)) / std_real)
